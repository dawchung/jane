...
function renderPrintSheet() {
  const authProfile = typeof window.AUTH_GATE?.getProfile === "function"
    ? window.AUTH_GATE.getProfile()
    : null;
  const printUserName = authProfile?.display_name || "____________";
  const aggregate = new Map();
  state.patients.forEach((patient) => {
    patient.cart.forEach((entry) => {
  ...
  const patientRows = state.patients.map((patient, index) => {
    ...
    const items = patient.cart.length
      ...
      : "尚未選購";
    return `<tr><td>${index + 1}</td><td>${patient.bed}</td><td>${patient.name}</td><td class="print-items">${items}</td><td>${total}</td><td>${patient.balance - total}</td><td class="signature-cell"></td></tr>`;
  }).join("");
  ...
  elements.printSheet.innerHTML = `
    ...
    <section>
      <div class="print-section-heading"><h2>採買品項總表</h2><span>列印人員：${escapeHtml(printUserName)}</span></div>
    ...
    <section>
      <h2>病人分發總表</h2>
      <table class="print-patient-table"><colgroup><col class="col-sequence"><col class="col-bed"><col class="col-name"><col class="col-items"><col class="col-amount"><col class="col-balance"><col class="col-patient-sign"></colgroup><thead><tr><th>序</th><th>床號</th><th>姓名</th><th>購物內容</th><th>金額</th><th>餘額</th><th>病人／家屬簽章</th></tr></thead>
      <tbody>${patientRows || '<tr><td colspan="7">本日尚無購物病人</td></tr>'}</tbody></table>
    </section>
